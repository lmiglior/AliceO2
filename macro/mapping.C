...
{
  fstream inputDec; //Fragment A mode frequencies
...
void mapping()
{
  ...
  const std::string inputGeom = "/home/o2flp/alice/O2/o2sim_geometry.root";
  ...
  int sizeVecDec=vecChipDec.size();
  fstream output_mapping;
  fstream output_histoname;
  fstream output_canvas;
  fstream output_trans;
  output_mapping.open("/home/o2flp/alice/output_raw/mapping.txt",ios::out);
  output_histoname.open("/home/o2flp/alice/output_raw/histoname.txt",ios::out);
  output_canvas.open("/home/o2flp/alice/output_raw/canvas.txt",ios::out);
  output_trans.open("/home/o2flp/alice/output_raw/trans.txt",ios::out);
  ...
  for (int i = 0; i < NChips; ++i) {
    ...
    for(int j=0;j<=sizeVecDec;j++){
      ...
      if (vecChipDec[j]==chipIDglo){
   printf("%-12d%-14d%-14d%-7d%-7d%-8d%-9d0x%-7x%-7d%-12d%-7d%-7d%-10d%-12d\n", chipIDglo, chipIDlocSW, chipIDlocHW, half, disk, layer, ruIDSW, ruIDHW, ruType, faceID, zone, connector, cableHW, chipOnRU);
	// cout<<"transID"<<cableHW<<endl;
	int transID = (int)cableHW;
	//	cout<<"converti?"<<vOut;

	std::ostringstream os1;
        os1 << "h" << half<<"-f"<<faceID <<"-d"<<disk<<"-z"<<zone<<"-trans"<<transID<<";Cols;Rows";
	output_mapping<<os1.str()<<endl;
	cout<<"Chips in decos : "<<os1.str()<<endl;
